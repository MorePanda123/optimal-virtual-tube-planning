...
function [X,U,dU,info] = getNMPCmatrices(time,MPC_vars,ModelParams,Xhor,Uhor,x0,u0, num, Drone)
...
stage(1).x0 = Drone(num).TrackMPC.des(1,:)'-x0;
...
function [Ck, lg] = getInequalityConstraints(MPC_vars,ModelParams,Xk,num,Drone,N_step)
    ...
    if num == 1
        ...
        for k =1:num-1
            ...
            Ck(k,1:3) = -2*[x0-ox,y0-oy,z0-oz];
            lg(k) = (x0-ox)^2 + (y0-oy)^2 + (z0-oz)^2 - rho^2 - ...
            2*[x0-ox, y0-oy, z0-oz]*[x0;y0;z0];
...
%             lg(k) =  2*rho^2 + c_xy'*p_o;
    ...
    for k=1:numObs
        ...
        oy = MPC_vars.obs(k,2);
        oz = MPC_vars.obs(k,3);
        lg2(k) = (x0-ox)^2 + (y0-oy)^2  - 7^2 - ...
            2*[x0-ox, y0-oy, 0]*[x0;y0;z0];
        Ck2(k,1:3) = -2*[x0-ox,y0-oy,0];
    end
