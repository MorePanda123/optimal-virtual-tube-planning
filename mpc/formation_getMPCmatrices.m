...
function [X,U,dU,info] = formation_getMPCmatrices(time,MPC_vars,ModelParams,Xhor,Uhor,x0,u0, num, Drone, pathNum)
...
stage(1).x0 = Drone(num).TrackMPC.des(pathNum,:)'-x0;
stage(1).u0 = zeros(3,1)-u0;
...
for i = 1:MPC_vars.N
    ...
    Pd = Drone(num).TrackMPC.des(pathNum,1:3)';
    Vd = Drone(num).TrackMPC.des(pathNum,4:6)';
    Ad = zeros(3,1);
    Pd1 = Pd;
    Vd1 = Vd;
%     Pd = ppval(Drone(num).TrackMPC.pp,time+(i-1)*MPC_vars.Ts);
%     Vd = ppval(Drone(num).TrackMPC.dpp,time+(i-1)*MPC_vars.Ts);
%     Ad = ppval(Drone(num).TrackMPC.ddpp,time+(i-1)*MPC_vars.Ts);
%     Pd1 = ppval(Drone(num).TrackMPC.pp,time+(i)*MPC_vars.Ts);
%     Vd1 = ppval(Drone(num).TrackMPC.dpp,time+(i)*MPC_vars.Ts);
%     Pd = DeBoorAlg(time+(i-1)*MPC_vars.Ts,Drone(num).TrackMPC.Knots,Drone(num).TrackMPC.Q,3)';
%     Vd = DeBoorAlg(time+(i-1)*MPC_vars.Ts,Drone(num).TrackMPC.Knots_d1,Drone(num).TrackMPC.Q_d1,2)';
%     Ad = DeBoorAlg(time+(i-1)*MPC_vars.Ts,Drone(num).TrackMPC.Knots_d2,Drone(num).TrackMPC.Q_d2,1)';
%     Pd1 = DeBoorAlg(time+(i)*MPC_vars.Ts,Drone(num).TrackMPC.Knots,Drone(num).TrackMPC.Q,3)';
%     Vd1 = DeBoorAlg(time+(i)*MPC_vars.Ts,Drone(num).TrackMPC.Knots_d1,Drone(num).TrackMPC.Q_d1,2)';
    Xdes_log(:,i) = [Pd;Vd];
    Udes_log(:,i) = Ad;
    ...
    [stage(i).Ck,stage(i).lg] = getInequalityConstraints(MPC_vars,ModelParams,Xk,num,Drone,i,pathNum);
    if num > 1
        stage(i).lg = stage(i).Ck * [Pd;Vd;Ad] - stage(i).lg;
    end
    numCol = length(Drone(num).poly(pathNum).A(:,1));
    Ck2 = [Drone(num).poly(pathNum).A zeros(numCol,6)];
    lg2 = [Drone(num).poly(pathNum).b];
    stage(i).Ck = [stage(i).Ck; -Ck2];
    stage(i).lg = [stage(i).lg; -Ck2*[Pd;Vd;Ad]+lg2];
...
% bounds
% [stage(i).lb, stage(i).ub] = getBounds(MPC_vars,ModelParams);
stage(i).ub = -MPC_vars.bounds(1:ModelParams.nx,1) + [Pd;Vd];
stage(i).lb = -MPC_vars.bounds(1:ModelParams.nx,2) + [Pd;Vd];
...
function [Ck, lg] = getInequalityConstraints(MPC_vars,ModelParams,Xk,num,Drone,N_step,pathNum)
    nx = ModelParams.nx;
    nu = ModelParams.nu;
