...
function [ theta, closestIdx] = findTheta(currentPosition,TrackCenter,traj_breaks,last_closestIdx)
...
posZ = currentPosition(3);
TrackX = TrackCenter(1,:);
TrackY = TrackCenter(2,:);
TrackZ = TrackCenter(3,:);
...
TrackZsmall = TrackZ(searchRegion);
distanceX = TrackXsmall - posX*ones(1,N_searchRegion);
distanceY = TrackYsmall - posY*ones(1,N_searchRegion);
distanceZ = TrackZsmall - posZ*ones(1,N_searchRegion);

squared_distance_array   = distanceX.^2 + distanceY.^2 + distanceZ.^2;
% find closest point
...
closestIdx = minIndex;
cosinus = dot([posX;posY;posZ] - trackCenterline(:,minIndex), trackCenterline(:,prevIdx) - trackCenterline(:,minIndex));
if (cosinus > 0)
    minIndex2 = prevIdx;
else
    minIndex2 = minIndex;
...
if e ~= 0
    cosinus = dot([posX;posY;posZ] - trackCenterline(:,minIndex2), trackCenterline(:,minIndex) - trackCenterline(:,minIndex2))/...
                 (norm([posX;posY;posZ] - trackCenterline(:,minIndex2))*norm(trackCenterline(:,minIndex) - trackCenterline(:,minIndex2)));
else
    cosinus = 0;
end
theta = traj_breaks(1,minIndex2);
theta = theta + cosinus*norm([posX;posY;posZ] - trackCenterline(:,minIndex2),2);
return