...
function [X,U,dU,info] = mao_getMPCmatrices(time,MPC_vars,ModelParams,Xhor,Uhor,x0,u0, num, Drone)
...
des_pos = ppval(Drone(num).TrackMPC.pp,time);
dis = norm(x0(1:3) - des_pos);
if dis > 1
    time = time - dis/10;
end

time = min(time, Drone(1).ts(end)-2);
stage(1).x0 = [ppval(Drone(num).TrackMPC.pp,time);ppval(Drone(num).TrackMPC.dpp,time)]-x0;
stage(1).u0 = ppval(Drone(num).TrackMPC.ddpp,time)-u0;
